...
export class ProductService extends BaseService {
  ...
  async getAllWithDetails(options?: {
    ...
    return this.safeQuery<ProductWithDetails[]>(async (client) => {
      ...
      if (result.data) {
        ...
        const productsWithDetails: ProductWithDetails[] = result.data.map(product => {
  ...
  async createProduct(data: ProductInsert): Promise<DatabaseResult<Product>> {
    ...
    return this.create<Product, ProductInsert>('products', {
      ...data,
  ...
  async updateProduct(id: string, data: ProductUpdate): Promise<DatabaseResult<Product>> {
    return this.update<Product, ProductUpdate>('products', id, {
      ...data,
      updated_at: this.formatDate(new Date())
    })
  }
  ...
  async getLowStockItems(): Promise<DatabaseResult<ProductWithDetails[]>> {
    return this.safeQuery<ProductWithDetails[]>(async (client) => {
      ...
      if (result.data) {
        const productsWithStatus = result.data.map(product => {
          const currentStock = product.current_stock || 0
          const minLevel = product.min_stock_level || 0
          const mauc = product.mauc || 0
        ...
        }).filter(product => product.stock_status === 'low-stock' || product.stock_status === 'out-of-stock')
  ...
  async createStockTransaction(data: StockTransactionInsert): Promise<DatabaseResult<StockTransaction>> {
    ...
    return this.safeQuery<StockTransaction>(async (client) => {
      ...
      if (product.data) {
        let newStock = product.data.current_stock || 0
        let newMAUC = product.data.mauc || 0
        ...
        switch (data.transaction_type) {
          case 'receive':
            ...
            if (data.unit_cost && newStock > 0) {
              const totalValue = (product.data.current_stock || 0) * (product.data.mauc || 0) + 
