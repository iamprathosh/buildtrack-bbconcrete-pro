...
export function AddPurchaseOrderDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { purchaseOrder: newOrder } = await response.json()
      
      // Convert date strings back to Date objects
      const orderWithDates: PurchaseOrder = {
        ...newOrder,
        orderDate: new Date(newOrder.orderDate),
        expectedDelivery: new Date(newOrder.expectedDelivery),
        actualDelivery: newOrder.actualDelivery ? new Date(newOrder.actualDelivery) : undefined,
        lastUpdated: new Date(newOrder.lastUpdated)
      }

      onOrderAdded(orderWithDates)
      resetForm()
